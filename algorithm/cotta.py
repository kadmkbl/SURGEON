...
class CoTTA(nn.Module):
    ...
    @staticmethod
    def collect_params(model, bn_only: bool, filter=None):
        ...
        for nm, m in model.named_modules():
            if not bn_only or isinstance(m, nn.BatchNorm2d):
    ...
    @staticmethod
    def configure_model(model, bn_only: bool, local_bn=True):
...
class CoTTA_ImageNet(nn.Module):
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        loss = (softmax_entropy(
            outputs, outputs_ema.detach(), symmetric=True)).mean(0)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        # Teacher update
...
@torch.jit.script
def softmax_entropy(x, x_ema, symmetric=False):# -> torch.Tensor:
