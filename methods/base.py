...
from util.dataset import prepare_imagenet_test_data, prepare_cifar10_test_data, prepare_cifar100_test_data
from algorithm.eata import compute_fishers
    ...
    def setup_optimizer(self):
        if self.cfg.OPTIM.METHOD == 'Adam':
            ...
            for param in self.params:
                base_params.append({"params": param, "lr": self.cfg.OPTIM.LR, "betas": (self.cfg.OPTIM.BETA, 0.999), "weight_decay": self.cfg.OPTIM.WD})
...
class TTAMethod_EATA(nn.Module):
    def __init__(self, cfg, model, num_classes):
        super().__init__()
        self.cfg = cfg
        self.model = model
        ...
        _, self.fisher_loader = prepare_data(
            'original', 5, cfg.TEST.BATCH_SIZE, workers=cfg.TEST.NUM_WORKERS, subset_size=2000)
        self.fishers = compute_fishers(self.params, self.model, self.fisher_loader, cfg.DEVICE)
        ...
        ### EATA Design
        if cfg.CORRUPTION.DATASET == 'cifar10_c':
            self.fisher_alpha = 1. # trade-off \beta for two losses (Eqn. 8) 
            self.e_margin = math.log(10) * 0.40 # hyper-parameter E_0 (Eqn. 3)
            self.d_margin = 0.4 # hyper-parameter \epsilon for consine simlarity thresholding (Eqn. 5)
        elif cfg.CORRUPTION.DATASET == 'cifar100_c':
            self.fisher_alpha = 2000. # trade-off \beta for two losses (Eqn. 8) 
            self.e_margin = math.log(100) * 0.40 # hyper-parameter E_0 (Eqn. 3)
            self.d_margin = 0.05 # hyper-parameter \epsilon for consine simlarity thresholding (Eqn. 5)
    ...
    def forward(self, x, adapt=True):
        ...
        if x[0].shape[0] == 1:  # single sample test-time adaptation
            ...
            for _ in range(self.steps):
                # outputs = self.forward_and_adapt(x)
                outputs, num_counts_2, num_counts_1, updated_probs, lr_weights = self.forward_and_adapt_eata(x, self.model, self.optimizer, self.fishers, self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin, skip_train=not adapt)
                self.num_samples_update_2 += num_counts_2
                self.num_samples_update_1 += num_counts_1
                self.reset_model_probs(updated_probs)

        return outputs, lr_weights
